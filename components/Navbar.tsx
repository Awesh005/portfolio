    ...
    <nav className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 ${scrolled ? 'py-4' : 'py-8'}`}>
      <div className="max-w-7xl mx-auto px-6">
        <div className={`flex justify-between items-center px-8 py-3 rounded-full transition-all duration-500 ${
          scrolled 
            ? (isDark ? 'bg-zinc-900/80 backdrop-blur-xl border border-white/10 shadow-2xl' : 'bg-white/80 backdrop-blur-xl border border-black/5 shadow-xl')
            : 'bg-transparent border border-transparent'
        }`}>
          <a href="#" className={`text-lg font-black tracking-tighter flex items-center gap-1 ${isDark ? 'text-white' : 'text-zinc-900'}`}>
          ...
          <div className="hidden md:flex gap-10 items-center">
            {navLinks.map((link) => (
              <a 
                key={link.name} 
                href={link.href}
                className={`text-[10px] font-black uppercase tracking-[0.2em] transition-all hover:tracking-[0.3em] ${isDark ? 'text-zinc-400 hover:text-white' : 'text-zinc-500 hover:text-zinc-900'}`}
              >
                {link.name}
              </a>
            ))}
            ...
            <button 
              onClick={toggleTheme}
              className={`p-2 rounded-full transition-all ${isDark ? 'text-zinc-400 hover:text-white' : 'text-zinc-500 hover:text-zinc-900'} hover:scale-110 active:scale-90`}
          ...
          <div className="flex md:hidden items-center gap-4">
            <button onClick={toggleTheme} className={`p-2 ${isDark ? 'text-white' : 'text-zinc-900'}`}>
              ...
              <div className="flex flex-col gap-8">
                {navLinks.map((link) => (
                  <a 
                    ...
                    href={link.href}
                    onClick={() => setMobileMenuOpen(false)}
                    className="text-3xl font-black tracking-tighter hover:text-indigo-500 transition-colors"
                  >
                    {link.name}
                  </a>
                ))}
